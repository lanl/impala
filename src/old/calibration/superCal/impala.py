...
def calibHier(setup):
    ...
    for m in range(1,setup.nmcmc):
        ...
        if m % 100 == 0 and m > 300:
            delta = 0#min(0.1, 1/np.sqrt(m+1)*5)
            for i in range(setup.nexp):
        ...
        if m % setup.decor == 0:
            for k in range(setup.p):
                ...
                alpha_tot = sum(alpha) -0.5 * setup.itl * (theta0_cand - theta0_prior_mean) @ theta0_prior_prec @ (theta0_cand - theta0_prior_mean).T + 0.5 * setup.itl * (theta0[m] - theta0_prior_mean) @ theta0_prior_prec @ (theta0[m] - theta0_prior_mean).T 
