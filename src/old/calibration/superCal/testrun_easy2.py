...
edots = np.hstack(meta.edot[:nExp].values)
temps = np.hstack(meta.temperature[:nExp].values)
...
# s2_ind = np.hstack([[v]*len(dat_all[v]) for v in list(range(nExp))] * 2)
s2_ind = np.hstack(
    [[v] * dat_all[v].shape[0] for v in range(nExp)]
    )

#%%  Pooled Model 
model = models.ModelPTW(temps, edots, consts, sh, True)
...
setup.setMCMC(3000,1000,1,10000000)
out = impala.calibPool(setup)

if False:
    plotsa = plots.PTW_Plotter(setup, out)
    plotsa.ptw_prediction_plots('./pool_predictions.pdf')
    plotsa.pairwise_theta_plot('./pool_pairwise.pdf')
    ...
    if __name__ == '__main__':
        ...
        #             )
        setup.setTemperatureLadder(1.1**np.arange(4))
        setup.setMCMC(30000, 10000, 1, 100)
        setup.set_max_clusters(40)
        out  = impala.calibClust(setup)
        plot = plots.PTW_Plotter(setup, out)
        plot.ptw_prediction_plots('./clust_predictions_test2.pdf')
        plot.pairwise_theta_plot('./clust_pairwise_test2.pdf')
        plot.cluster_matrix_plot('./clust_clusters_test2.pdf')
