...
def chol_sample_1per_constraints(
    means, covs, cf, bounds_mat, bounds_keys, bounds
):
    ...
    while np.any(~good):
        cand[np.where(~good)] = +means[~good] + np.einsum(
            "ijk,ik->ij",
            chols[~good],
            normal(size=((~good).sum(), means.shape[1])),
        )
        good[~good] = cf(
            tran_unif(cand[~good], bounds_mat, bounds_keys), bounds
        )
    return cand


def chol_sample_nper_constraints(
    means, covs, n, cf, bounds_mat, bounds_keys, bounds
):
    """Sample with constraints.  If fail constraints, resample."""
    chols = cholesky(covs)
    ...
    for i in range(cand.shape[0]):
        ...
        while np.any(~goodi):
            cand[i, np.where(~goodi)[0]] = +means[i] + np.einsum(
                "ik,nk->ni",
                chols[i],
                normal(size=((~goodi).sum(), means.shape[1])),
            )
            goodi[np.where(~goodi)[0]] = cf(
                tran_unif(
                    cand[i, np.where(~goodi)[0]], bounds_mat, bounds_keys
                ),
                bounds,
            )
    return cand
...
def calibHier(setup):
    ...
    while np.any(~good):
        theta0_start[np.where(~good)] = initfunc_unif(
            size=[(~good).sum(), setup.p]
        )
        good[np.where(~good)] = setup.checkConstraints(
            tran_unif(
                theta0_start[np.where(~good)],
                setup.bounds_mat,
                setup.bounds.keys(),
            ),
            setup.bounds,
