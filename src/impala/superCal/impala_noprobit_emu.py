...
class CalibSetup:
    ...
    * setClusterPriors
    ...
    def addVecExperiments(
        ...
        if theta_ind is not None:
            if not is_valid_mapping(np.array(theta_ind), np.array(s2_ind)):
                print('Warning: Cannot have multiple thetas with shared measurement error.')
...
def calibHier_v2(setup):
    ...
    theta0[0] = theta0_start
    Sigma0[0] = setup.Sigma0_prior_scale/(setup.Sigma0_prior_df - setup.p - 1) #initialize at prior mean

        
    wt_mat = [None] * setup.nexp
    for i in range(setup.nexp):
        wt_mat[i] = setup.wt[i]
        if np.any(np.asarray([len(np.unique(wt_mat[i][s2_which_mat[i][j]])) for j in range(len(s2_which_mat[i]))])) != 1:
            if setup.models[i].s2 == 'gibbs' | setup.models[i].s2 == 'gibbs_trunc':
                setup.models[i].s2 = 'MH'
                print('Gibbs sampling for s2 only valid if weights are the same for all observations with same s2. Reverting to MH. ')
    ...
    for i in range(setup.nexp):
        ...
        for t in range(setup.ntemps):
            ...
            for j in range(setup.ntheta[i]):
                marg_lik_cov_curr[i][t][j] = setup.models[i].lik_cov_inv_v2(np.exp(s2_stretched[theta_which_mat[i][j]]), s2_which_mat[i][j]) 
                llik_curr[i][t][j] = setup.models[i].llik_v2(setup.ys[i][theta_which_mat[i][j]], pred_curr[i][t][theta_which_mat[i][j]], marg_lik_cov_curr[i][t][j], wt_mat[i][theta_which_mat[i][j]]) 
                #this isnt getting nthetas correct, probably need to change models script...
                # there should be a separate likelihood evaluation (with separate covariance) 
                # for every i, t, ntheta. In diagonal case, we could vectorize over t ntheta...
                # for now, break into separate calls.  Later may be worthwhile to try to vectorize more.
    ...
    for m in pbar(range(1, setup.nmcmc)):
        ...
        for i in range(setup.nexp):
            # Find new candidate values for theta
            theta_eval_mat[i][:] = theta[i][m-1].reshape(setup.ntemps * setup.ntheta[i], setup.p).copy()
            ...
            for t in range(setup.ntemps):
                for j in range(setup.ntheta[i]):
                    llik_cand[i][t][j] = setup.models[i].llik_v2(setup.ys[i][theta_which_mat[i][j]], pred_cand[i][t][theta_which_mat[i][j]], marg_lik_cov_curr[i][t][j], wt_mat[i][theta_which_mat[i][j]]) 
        ...
        for i in range(setup.nexp):
            if setup.models[i].s2=='gibbs':
                # ## gibbs update s2       
                dev_sq = ((pred_curr[i] - setup.ys[i])**2 @ s2_ind_mat[i]) #(ntemps x ns2[i])
                for t in range(setup.ntemps):
                    log_s2[i][m][t] = np.log(1 / np.random.gamma(
                        (itl_mat_s2[i][t] * (setup.ny_s2[i] / 2 + setup.ig_a[i] + 1) - 1).astype('float64'),
                        (1 / (itl_mat_s2[i][t] * (setup.ig_b[i] + dev_sq[t].flatten() / 2))).astype('float64')).astype('float64'))
                ...
                for t in range(setup.ntemps):
                    log_s2[i][m][t] = np.log(1 / np.random.gamma(
                        (itl_mat_s2[i][t] * (setup.ny_s2[i] / 2 + setup.ig_a[i] + 1) - 1).astype('float64'),
                        (1 / (itl_mat_s2[i][t] * (setup.ig_b[i] + dev_sq[t].flatten() / 2))).astype('float64')).astype('float64'))
                    s2_is_valid = (log_s2[i][m][t] >= np.log(setup.sd_lower[i]**2)) * (log_s2[i][m][t] <= np.log(setup.sd_upper[i]**2))
                    
                    ct = 0
                    while np.any(~s2_is_valid):
                        sub = np.where(~s2_is_valid)
                        log_s2[i][m][t][sub] = np.log(1 / np.random.gamma(
                            (itl_mat_s2[i][t][sub] * (setup.ny_s2[i][sub] / 2 + setup.ig_a[i][sub] + 1) - 1).astype('float64'),
                            (1 / (itl_mat_s2[i][t][sub] * (setup.ig_b[i][sub] + dev_sq[t].flatten()[sub] / 2))).astype('float64')).astype('float64'))
                        s2_is_valid = (log_s2[i][m][t] >= np.log(setup.sd_lower[i]**2)) * (log_s2[i][m][t] <= np.log(setup.sd_upper[i]**2))
                        ct = ct + 1
                        if ct >= 50:
                            log_s2[i][m][t][log_s2[i][m][t] < np.log(setup.sd_lower[i]**2)] = np.log(setup.sd_lower[i]**2)[log_s2[i][m][t] < np.log(setup.sd_lower[i]**2)]
                            log_s2[i][m][t][log_s2[i][m][t] > np.log(setup.sd_upper[i]**2)] = np.log(setup.sd_upper[i]**2)[log_s2[i][m][t] > np.log(setup.sd_upper[i]**2)]
                            s2_is_valid = (log_s2[i][m][t] >= np.log(setup.sd_lower[i]**2)) * (log_s2[i][m][t] <= np.log(setup.sd_upper[i]**2))
                ...
                for t in range(setup.ntemps):
                    marg_lik_cov_candi[t] = [None] * setup.ntheta[i]
                    for j in range(setup.ntheta[i]):
                        marg_lik_cov_candi[t][j] = setup.models[i].lik_cov_inv_v2(
                            np.exp(ls2_candi[t, setup.s2_ind[i]])[
                                setup.s2_ind[i] == j
                            ],
                            s2_which_mat[i][j]
...
def calibPool_v2(setup):
    ...
    s2_which_mat = [
        [np.where(s2_ind_mat[i][:,j])[0] for j in range(s2_ind_mat[i].shape[1])]
        for i in range(setup.nexp)
        ]
    ...
    for i in range(setup.nexp):
        ...
        if np.any(np.asarray([len(np.unique(wt_mat[i][s2_which_mat[i][j]])) for j in range(len(s2_which_mat[i]))])) != 1:
            if setup.models[i].s2 == 'gibbs' | setup.models[i].s2 == 'gibbs_trunc':
                ...
                print('Gibbs sampling for s2 only valid if weights are the same for all observations with same s2. Reverting to fixed s2. ')
    ...
    for i in range(setup.nexp):
        marg_lik_cov_curr[i] = [None] * setup.ntemps
        for t in range(setup.ntemps):
            marg_lik_cov_curr[i][t] = setup.models[i].lik_cov_inv_v2(np.exp(log_s2[i][0, t, setup.s2_ind[i]])[setup.s2_ind[i]],setup.s2_ind[i])
            # ask around: is list of lists lookup slow?? ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    for i in range(setup.nexp):
        ...
        #((pred_curr[i] - setup.ys[i])**2 @ s2_ind_mat[i] / s2[i][0]).sum(axis = 1)
        for t in range(setup.ntemps):
            llik_curr[i, t] = setup.models[i].llik_v2(setup.ys[i], pred_curr[i][t], marg_lik_cov_curr[i][t], wt_mat[i])
    ...
    for m in pbar(range(1, setup.nmcmc)):
        ...
        for i in range(setup.nexp):
            ...
            if setup.models[i].nd>0: # update discrepancy
                for t in range(setup.ntemps):
                    discrep_vars[i][m][t] = setup.models[i].discrep_sample_v2(setup.ys[i], pred_curr[i][t], marg_lik_cov_curr[i][t], setup.itl[t], wt_mat[i])
            ...
            if setup.models[i].nd>0 or setup.models[i].stochastic:
                for t in range(setup.ntemps):
                    llik_curr[i, t] = setup.models[i].llik_v2(setup.ys[i] - discrep_curr[i][t], pred_curr[i][t], marg_lik_cov_curr[i][t], wt_mat[i])
        ...
        if np.any(good_values):
            ...
            for i in range(setup.nexp):
                ...
                for t in range(setup.ntemps):
                    llik_cand[i, t] = setup.models[i].llik_v2(setup.ys[i] - discrep_curr[i][t], pred_cand[i][t], marg_lik_cov_curr[i][t], wt_mat[i])
            ...
            for k in range(setup.p):
                ...
                if np.any(good_values):
                    ...
                    for i in range(setup.nexp):
                        ...
                        for t in range(setup.ntemps):
                            llik_cand[i, t] = setup.models[i].llik_v2(setup.ys[i] - discrep_curr[i][t], pred_cand[i][t], marg_lik_cov_curr[i][t], wt_mat[i])#(((pred_cand[i] - setup.ys[i])**2 @ s2_ind_mat[i]) / s2[i][m-1]).sum(axis = 1)
        ...
        for i in range(setup.nexp):
            ...
            if setup.models[i].s2=='gibbs':
                ...
                for t in range(setup.ntemps):
                    log_s2[i][m][t] = np.log(1 / np.random.gamma(
                        (itl_mat[i][t] * (setup.ny_s2[i] / 2 + setup.ig_a[i] + 1) - 1).astype('float64'),
                        (1 / (itl_mat[i][t] * (setup.ig_b[i] + dev_sq[t].flatten() / 2))).astype('float64')).astype('float64'))
                    marg_lik_cov_curr[i][t] = setup.models[i].lik_cov_inv_v2(np.exp(log_s2[i][m][t])[setup.s2_ind[i]],setup.s2_ind[i])
                    llik_curr[i, t] = setup.models[i].llik_v2(setup.ys[i] - discrep_curr[i][t], pred_curr[i][t], marg_lik_cov_curr[i][t], wt_mat[i])
                ...
                for t in range(setup.ntemps):
                    ...
                    while np.any(~s2_is_valid):
                        sub = np.where(~s2_is_valid)
                        log_s2[i][m][t][sub] = np.log(1 / np.random.gamma(
                            (itl_mat[i][t][sub] * (setup.ny_s2[i][sub] / 2 + setup.ig_a[i][sub] + 1) - 1).astype('float64'),
                            (1 / (itl_mat[i][t][sub] * (setup.ig_b[i][sub] + dev_sq[t].flatten()[sub] / 2))).astype('float64')).astype('float64'))
                        s2_is_valid = (log_s2[i][m][t] >= np.log(setup.sd_lower[i]**2)) * (log_s2[i][m][t] <= np.log(setup.sd_upper[i]**2))
                        ct = ct + 1
                        if ct >= 50:
                            log_s2[i][m][t][log_s2[i][m][t] < np.log(setup.sd_lower[i]**2)] = np.log(setup.sd_lower[i]**2)[log_s2[i][m][t] < np.log(setup.sd_lower[i]**2)]
                ...
                marg_lik_cov_candi = [None] * setup.ntemps
                for t in range(setup.ntemps):
                    marg_lik_cov_candi[t] = setup.models[i].lik_cov_inv_v2(np.exp(ls2_candi[t])[setup.s2_ind[i]],setup.s2_ind[i])
                    llik_candi[t] = setup.models[i].llik_v2(setup.ys[i] - discrep_curr[i][t], pred_curr[i][t], marg_lik_cov_candi[t], wt_mat[i])
