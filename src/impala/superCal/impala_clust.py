...
### - changing initial theta0, Sigma0 defaults
...
def calibClust_v2(setup):
    ...
    # boolean with dimension [i] x [ylens[i], ntheta[i]] indicating which theta to use for each output
    theta_ind_mat = [(setup.theta_ind[i][:, None] == range(setup.ntheta[i])) for i in range(setup.nexp)]
    ...
    # repeated inverse temperatures of dimension [i] x [ntemps, ns2[i]]
    itl_mat_s2 = [np.ones((setup.ntemps, setup.ns2[i])) * setup.itl.reshape(-1, 1) for i in range(setup.nexp)]
    ...
    Sigma0 = np.empty([setup.nmcmc, setup.ntemps, setup.p, setup.p])
    Sigma0[0] = setup.Sigma0_prior_scale/(setup.Sigma0_prior_df - setup.p - 1)#Changed initialization
    ...
    for i in range(setup.nexp):
        theta_hist[i][0] = (
            theta[0, theta_unravel[i], delta[i][0].ravel()].reshape(setup.ntemps, setup.ntheta[i], setup.p) ### edited ns2 -> ntheta
            )
    theta_eval = np.empty(theta_wide_shape)
    ...
    for i in range(setup.nexp):
        ...
        for t in range(setup.ntemps):
            ...
            for j in range(setup.ntheta[i]):
                ...
                llik_curr_theta[i][t][j] = setup.models[i].llik_v2(setup.ys[i][theta_which_mat[i][j]], pred_curr_theta[i][t][theta_which_mat[i][j]], marg_lik_cov_curr[i][t][j], wt_mat[i][theta_which_mat[i][j]]) 
        llik_cand_theta[i] = llik_curr_theta[i].copy()

        ### Initialize predictions for clusters
        pred_curr_delta[i] = setup.models[i].eval(
            tran_unif(theta[0].reshape(-1, setup.p), setup.bounds_mat, setup.bounds.keys()), True,
            ).reshape(setup.ntemps, setup.nclustmax, setup.y_lens[i]) #this is now smaller because it uses more efficient pool = True code
        llik_curr_delta[i] = np.empty([setup.ntemps, setup.nclustmax, setup.ntheta[i]])
        for t in range(setup.ntemps):
            s2_stretched = log_s2[i][0][t,setup.s2_ind[i]]
            for j in range(setup.ntheta[i]):
                for k in range(setup.nclustmax):
                    llik_curr_delta[i][t][k][j] = setup.models[i].llik_v2(setup.ys[i][theta_which_mat[i][j]], pred_curr_delta[i][t][k][theta_which_mat[i][j]], marg_lik_cov_curr[i][t][j], wt_mat[i][theta_which_mat[i][j]]) 
    ...
    for m in range(1, setup.nmcmc):
        ...
        for i in range(setup.nexp):
            theta_hist[i][m] = (
                theta[m-1, theta_unravel[i], delta[i][m].ravel()
                    ].reshape(setup.ntemps, setup.ntheta[i], setup.p) #added theta_hist update 9/19/25
                )
            pred_curr_theta[i][:] = setup.models[i].eval(
        ...
        for i in range(setup.nexp):
            if setup.models[i].s2=='gibbs':
                dev_sq = ((pred_curr_theta[i] - setup.ys[i])**2 @ s2_ind_mat[i])#(ntemps x ns2[i])
                for t in range(setup.ntemps):
                ...
                for t in range(setup.ntemps):
                    ...
                    for j in range(setup.ntheta[i]):
                        marg_lik_cov_curr[i][t][j] = setup.models[
                            i
                        ].lik_cov_inv_v2(np.exp(s2_stretched[s2_which_mat[i][j]]), s2_which_mat[i][j])
                        llik_curr_theta[i][t][j] = setup.models[i].llik(
                            setup.ys[i][s2_which_mat[i][j]],
                            pred_curr_theta[i][t][s2_which_mat[i][j]],
                            marg_lik_cov_curr[i][t][j],
        ...
        if m > setup.start_temper and setup.ntemps > 1:
            for _ in range(setup.nswap):
                ...
                sw_alpha[:] = sw_alpha + (setup.itl[sw.T[1]] - setup.itl[sw.T[0]]) * (
                    ...
                    - gamma_logpdf(eta[m][sw.T[1]], setup.eta_prior_shape, setup.eta_prior_rate)
                    + gamma_logpdf(eta[m][sw.T[0]], setup.eta_prior_shape, setup.eta_prior_rate)
                    )
                for i in range(setup.nexp):
                ...
                for tt in sw[np.where(sw[np.log(uniform(size = setup.nswap_per)) < sw_alpha])[0]]:
                    count_temper[tt[0], tt[1]] = count_temper[tt[0], tt[1]] + 1 
                    eta[m,tt[0]], eta[m,tt[1]] = eta[m,tt[1]].copy(), eta[m,tt[0]].copy() #previously not tempering over eta correctly
