...
class PTW_Yield_Stress(BaseModel):
    ...
    def value(self, edot):
        ...
        saturation2 = mp.s0 * np.exp(
            mp.beta * (-mp.lgamma + np.log(edot / xiDot))
        )
        ...
        eArg2 = (
            (eps * mp.p * mp.theta)[ind]
            / (mp.s0 - tau_y)[ind]
            / (np.exp(eArg1) - 1.0)
        )  # eArg1 already subsetted by ind
        if np.any(
            (1.0 - (1.0 - np.exp(-eArg1)) * np.exp(-eArg2)) <= 0
        ) or np.any(np.isinf(1.0 - (1.0 - np.exp(-eArg1)) * np.exp(-eArg2))):
            print("bad")
        theLog = np.log(1.0 - (1.0 - np.exp(-eArg1)) * np.exp(-eArg2))
        scaled_stress[ind] = (
            tau_s[ind] + (mp.s0[ind] - tau_y[ind]) * theLog / mp.p[ind]
        )
