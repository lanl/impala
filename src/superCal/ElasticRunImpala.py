...
# initialize setup
setup = impala.CalibSetup(bounds, constraint_funcion)

# create appropriate model objects
model_ftilde = models.ModelBassPca_func(emu_ftilde, input_names) # for aligned data
model_vv = models.ModelBassPca_func(emu_vv, input_names) # for warping functions
...
ftilde_pred_obs = setup.models[0].eval(impala.tran_unif(out.theta[uu,0,:], setup.bounds_mat, setup.bounds.keys())) * ftilde_sd
...
vpred_obs = setup.models[1].eval(impala.tran_unif(out.theta[uu,0,:], setup.bounds_mat, setup.bounds.keys())) * vv_train_sd
...
def unwarp(gam, ftilde, x):
    ...
    x_unwarped = impala.unnormalize(gam, min(x), max(x))
