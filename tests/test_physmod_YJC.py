...
class TestJCYieldStress(unittest.TestCase):
    ...
    def test_isothermal_lowrate(self):
        ...
        self.model_yjc.initialize_state(T=np.array([298.]))
    ...
    def test_stress_highrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.28179]), edot = np.array([1e-2]), nhist = 1000)
        self.model_yjc.initialize(self.params, self.consts)
        self.model_yjc.initialize_state(T=np.array([298.]))
        results_yjc = self.model_yjc.compute_state_history(shist)
        # result format
    ...
    def test_stress_midrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.28895]), edot = np.array([1e-4]), nhist = 1000)
        self.model_yjc.initialize(self.params, self.consts)
        self.model_yjc.initialize_state(T=np.array([298.]))
        results_yjc = self.model_yjc.compute_state_history(shist)
        # result format
    ...
    def test_stress_lowrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.28928]), edot = np.array([1e-7]), nhist = 1000)
        self.model_yjc.initialize(self.params, self.consts)
        self.model_yjc.initialize_state(T=np.array([298.]))
        results_yjc = self.model_yjc.compute_state_history(shist)
        # result format
