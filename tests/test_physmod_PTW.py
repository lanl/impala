...
class TestPTWYieldStress_Constg0Tm(unittest.TestCase):
    ...
    def test_isothermal_lowrate(self):
        ...
        """
        shist = pmh.generate_strain_history_new(emax = np.array([1.0]), edot = np.array([1e-7]), nhist = 1000)
        self.model_ptw_cg0Tm.initialize(self.params, self.consts)
        self.model_ptw_cg0Tm.initialize_state(T=np.array([298.]))
    ...
    def test_adiabatic_highrate(self):
        ...
        """
        shist = pmh.generate_strain_history_new(emax = np.array([1.0]), edot = np.array([1e-2]), nhist = 1000)
        self.model_ptw_cg0Tm.initialize(self.params, self.consts)
        self.model_ptw_cg0Tm.initialize_state(T=np.array([298.]))
        results_ptw_cg0Tm = self.model_ptw_cg0Tm.compute_state_history(shist)
        # result format
    ...
    def test_stress_highrate(self):
        ...
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.244376]), edot = np.array([1e0]), nhist = 1000)
        self.model_ptw_cg0Tm.initialize(self.params, self.consts)
        self.model_ptw_cg0Tm.initialize_state(T=np.array([298.]))
        results_ptw_cg0Tm = self.model_ptw_cg0Tm.compute_state_history(shist)
        # result format
        ...
        rel_diff = abs(stress_emax-stress_FLAG)/stat.mean([stress_emax,stress_FLAG])
    ...
    def test_stress_midhighrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.382749]), edot = np.array([1e-2]), nhist = 1000)
        self.model_ptw_cg0Tm.initialize(self.params, self.consts)
        self.model_ptw_cg0Tm.initialize_state(T=np.array([298.]))
        results_ptw_cg0Tm = self.model_ptw_cg0Tm.compute_state_history(shist)
        # result format
    ...
    def test_stress_midlowrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.388339]), edot = np.array([1e-4]), nhist = 1000)
        self.model_ptw_cg0Tm.initialize(self.params, self.consts)
        self.model_ptw_cg0Tm.initialize_state(T=np.array([298.]))
        results_ptw_cg0Tm = self.model_ptw_cg0Tm.compute_state_history(shist)
        # result format
    ...
    def test_stress_lowrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.371143]), edot = np.array([1e-6]), nhist = 1000)
        self.model_ptw_cg0Tm.initialize(self.params, self.consts)
        self.model_ptw_cg0Tm.initialize_state(T=np.asarray([298.]))
        results_ptw_cg0Tm = self.model_ptw_cg0Tm.compute_state_history(shist)
        # result format
        # [time, strain, stress, temp, shear_mod, density]
        # Assess stress at the emax
...
class TestPTWYieldStress_SimpShearConstTm(unittest.TestCase):
    ...
    def test_isothermal_lowrate(self):
        ...
        self.model_ptw_ss_cTm.initialize_state(T=np.array([298.]))
    ...
    def test_stress_highrate(self):
        ...
        the FLAG result
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.244463]), edot = np.array([1e0]), nhist = 1000)
        self.model_ptw_ss_cTm.initialize(self.params, self.consts)
        self.model_ptw_ss_cTm.initialize_state(T=np.array([620.]))
        results_ptw_ss_cTm = self.model_ptw_ss_cTm.compute_state_history(shist)
        # result format
        # [time, strain, stress, temp, shear_mod, density]
        # Assess stress at the emax
    ...
    def test_stress_midhighrate(self):
        ...
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.383542]), edot = np.array([1e-2]), nhist = 1000)
        self.model_ptw_ss_cTm.initialize(self.params, self.consts)
        self.model_ptw_ss_cTm.initialize_state(T=np.array([298.]))
        results_ptw_ss_cTm = self.model_ptw_ss_cTm.compute_state_history(shist)
        # result format
    ...
    def test_stress_midlowrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.388302]), edot = 1e-4, nhist = 1000)
        self.model_ptw_ss_cTm.initialize(self.params, self.consts)
        self.model_ptw_ss_cTm.initialize_state(T=np.array([298.]))
        results_ptw_ss_cTm = self.model_ptw_ss_cTm.compute_state_history(shist)
        # result format
    ...
    def test_stress_lowrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.385416]), edot = np.array([1e-6]), nhist = 1000)
        self.model_ptw_ss_cTm.initialize(self.params, self.consts)
        self.model_ptw_ss_cTm.initialize_state(T=np.array([298.]))
        results_ptw_ss_cTm = self.model_ptw_ss_cTm.compute_state_history(shist)
        # result format
...
class TestPTWYieldStress_SteinShearConstTm(unittest.TestCase):
    ...
    def test_isothermal_lowrate(self):
        ...
        self.model_ptw_sg_cTm.initialize_state(T=np.array([298.]))
    ...
    def test_stress_highrate(self):
        ...
        the FLAG result
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.244632]), edot = np.array([1e0]), nhist = 1000)
        self.model_ptw_sg_cTm.initialize(self.params, self.consts)
        self.model_ptw_sg_cTm.initialize_state(T=np.array([617.]))
        results_ptw_sg_cTm = self.model_ptw_sg_cTm.compute_state_history(shist)
        # result format
        # [time, strain, stress, temp, shear_mod, density]
        # Assess stress at the emax
    ...
    def test_stress_midhighrate(self):
        ...
        create significant differences in computed stress.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.384834]), edot = np.array([1e-2]), nhist = 1000)
        self.model_ptw_sg_cTm.initialize(self.params, self.consts)
        self.model_ptw_sg_cTm.initialize_state(T=np.array([298.]))
        results_ptw_sg_cTm = self.model_ptw_sg_cTm.compute_state_history(shist)
        # result format
    ...
    def test_stress_midlowrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.388306]), edot = np.array([1e-4]), nhist = 1000)
        self.model_ptw_sg_cTm.initialize(self.params, self.consts)
        self.model_ptw_sg_cTm.initialize_state(T=np.array([298.]))
        results_ptw_sg_cTm = self.model_ptw_sg_cTm.compute_state_history(shist)
        # result format
    ...
    def test_stress_lowrate(self):
        ...
        zero plastic strain.
        """
        shist = pmh.generate_strain_history_new(emax = np.array([0.388966]), edot = np.array([1e-6]), nhist = 1000)
        self.model_ptw_sg_cTm.initialize(self.params, self.consts)
        self.model_ptw_sg_cTm.initialize_state(T=np.array([298.]))
        results_ptw_sg_cTm = self.model_ptw_sg_cTm.compute_state_history(shist)
        # result format
