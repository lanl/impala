...
class TestPTWYieldStress_Constg0Tm(unittest.TestCase):
    ...
    def test_isothermal_lowrate(self):
        ...
        self.model_vec_ptw_cg0Tm.initialize_state(T=298.)
        results_vec_ptw_cg0Tm = self.model_vec_ptw_cg0Tm.compute_state_history(shist_vec)
        # result format
        # [time, strain, stress, temp, shear_mod, density]

        # Compare results at beginning and end between original and vectorized
        print("Temp. at t=0: {0:.2f} K and t=end: {1:.2f} K".format(results_vec_ptw_cg0Tm[0][3][0],results_vec_ptw_cg0Tm[-1][3][0]))
        ...
        self.assertEqual(results_vec_ptw_cg0Tm[0][3],results_vec_ptw_cg0Tm[-1][3])
    ...
    def test_adiabatic_highrate(self):
        ...
        print("Temp. at t=0: {0:.2f} K and t=end: {1:.2f} K".format(results_vec_ptw_cg0Tm[0][3][0],results_vec_ptw_cg0Tm[-1][3][0]))
        self.assertEqual(results_ptw_cg0Tm[0][3],results_vec_ptw_cg0Tm[0][3])
        np.testing.assert_allclose(results_ptw_cg0Tm[-1][3],results_vec_ptw_cg0Tm[-1][3])
        self.assertNotEqual(results_vec_ptw_cg0Tm[0][3],results_vec_ptw_cg0Tm[-1][3])
    def test_stress_highrate(self):
        ...
        print("Eps/Sig at t=0: {0:.2f}/{1:.2f} MPa and"
              " t=end: {2:.2f}/{3:.2f} MPa".format(
                results_vec_ptw_cg0Tm[0][1][0],results_vec_ptw_cg0Tm[0][2][0]*1e5,
                results_vec_ptw_cg0Tm[-1][1][0],results_vec_ptw_cg0Tm[-1][2][0]*1e5))
        np.testing.assert_allclose(results_ptw_cg0Tm,results_vec_ptw_cg0Tm[:,:,0])

    def test_stress_midhighrate(self):
        """
        PTW, constant g0 and Tm, mid-high rate 1e4/s, compare vectorized w/ original.
    ...
    def test_stress_midlowrate(self):
        """
        PTW, constant g0 and Tm, low-mid rate 1e2/s, compare vectorized w/ original.
...
class TestPTWYieldStress_SimpShearConstTm(unittest.TestCase):
    ...
    def test_isothermal_lowrate(self):
        ...
        self.model_vec_ptw_ss_cTm.initialize_state(T=298.)
        results_vec_ptw_ss_cTm = self.model_vec_ptw_ss_cTm.compute_state_history(shist_vec)
        # result format
        # [time, strain, stress, temp, shear_mod, density]

        # Compare results at beginning and end between original and vectorized
        print("Temp. at t=0: {0:.2f} K and t=end: {1:.2f} K".format(results_vec_ptw_ss_cTm[0][3][0],results_vec_ptw_ss_cTm[-1][3][0]))
        self.assertEqual(results_ptw_ss_cTm[0][3],results_vec_ptw_ss_cTm[0][3])
        self.assertEqual(results_ptw_ss_cTm[-1][3],results_vec_ptw_ss_cTm[-1][3])
        self.assertEqual(results_vec_ptw_ss_cTm[0][3],results_vec_ptw_ss_cTm[-1][3])
    ...
    def test_adiabatic_highrate(self):
        ...
        self.assertEqual(results_ptw_ss_cTm[0][3],results_vec_ptw_ss_cTm[0][3])
        self.assertEqual(results_ptw_ss_cTm[-1][3],results_vec_ptw_ss_cTm[-1][3])
        self.assertNotEqual(results_vec_ptw_ss_cTm[0][3],results_vec_ptw_ss_cTm[-1][3])

    def test_stress_highrate(self):
        """
        PTW, constant Tm, simple shear mod, high rate 1e6/s, temp.=620K, compare vectorized w/ original.
        ...
        print("Eps/Sig at t=0: {0:.2f}/{1:.2f} MPa and"
              " t=end: {2:.2f}/{3:.2f} MPa".format(
                results_vec_ptw_ss_cTm[0][1][0],results_vec_ptw_ss_cTm[0][2][0]*1e5,
                results_vec_ptw_ss_cTm[-1][1][0],results_vec_ptw_ss_cTm[-1][2][0]*1e5))
        np.testing.assert_allclose(results_ptw_ss_cTm,results_vec_ptw_ss_cTm[:,:,0])

    def test_stress_midhighrate(self):
        """
        PTW, constant Tm, simple shear mod, mid-high rate 1e4/s, compare vectorized w/ original.
    ...
    def test_stress_midlowrate(self):
        """
        PTW, constant Tm, simple shear mod, low-mid rate 1e2/s, compare vectorized w/ original.
    ...
    def test_stress_lowrate(self):
        """
        PTW, constant Tm, simple shear mod, low rate 1e0/s, compare vectorized w/ original.
...
class TestPTWYieldStress_SteinShearConstTm(unittest.TestCase):
    ...
    def test_isothermal_lowrate(self):
        ...
        self.model_vec_ptw_sg_cTm.initialize_state(T=298.)
        results_vec_ptw_sg_cTm = self.model_vec_ptw_sg_cTm.compute_state_history(shist_vec)
        # result format
        # [time, strain, stress, temp, shear_mod, density]

        # Compare results at beginning and end between original and vectorized
        print("Temp. at t=0: {0:.2f} K and t=end: {1:.2f} K".format(results_vec_ptw_sg_cTm[0][3][0],results_vec_ptw_sg_cTm[-1][3][0]))
        self.assertEqual(results_ptw_sg_cTm[0][3],results_vec_ptw_sg_cTm[0][3])
        self.assertEqual(results_ptw_sg_cTm[-1][3],results_vec_ptw_sg_cTm[-1][3])
        self.assertEqual(results_vec_ptw_sg_cTm[0][3],results_vec_ptw_sg_cTm[-1][3])
    ...
    def test_adiabatic_highrate(self):
        ...
        self.assertEqual(results_ptw_sg_cTm[0][3],results_vec_ptw_sg_cTm[0][3])
        self.assertEqual(results_ptw_sg_cTm[-1][3],results_vec_ptw_sg_cTm[-1][3])
        self.assertNotEqual(results_vec_ptw_sg_cTm[0][3],results_vec_ptw_sg_cTm[-1][3])

    def test_stress_highrate(self):
        """
        PTW, constant Tm, Steinberg-Guinan shear mod, high rate 1e6/s, temp.=617K, compare vectorized w/ original.
        ...
        print("Eps/Sig at t=0: {0:.2f}/{1:.2f} MPa and"
              " t=end: {2:.2f}/{3:.2f} MPa".format(
                results_vec_ptw_sg_cTm[0][1][0],results_vec_ptw_sg_cTm[0][2][0]*1e5,
                results_vec_ptw_sg_cTm[-1][1][0],results_vec_ptw_sg_cTm[-1][2][0]*1e5))
        np.testing.assert_allclose(results_ptw_sg_cTm,results_vec_ptw_sg_cTm[:,:,0])

    def test_stress_midhighrate(self):
        """
        PTW, constant Tm, Steinberg-Guinan shear mod, mid-high rate 1e4/s, compare vectorized w/ original.
    ...
    def test_stress_midlowrate(self):
        """
        PTW, constant Tm, Steinberg-Guinan shear mod, low-mid rate 1e2/s, compare vectorized w/ original.
    ...
    def test_stress_lowrate(self):
        """
        PTW, constant Tm, Steinberg-Guinan shear mod, low rate 1e0/s, compare vectorized w/ original.
