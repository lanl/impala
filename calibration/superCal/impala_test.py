...
def calibHier(setup):
    """ Hierarchical Calibration Routine (Thetas for everyone!) """
    #np.random.seed(1511)
            ...
            for i in range(setup.nexp):
                for it in range(setup.ntheta[i]):
                    for t in range(setup.ntemps):
                        ...
                        S[i][t,it,:,:] = (cov[i][t,it,:,:]*cc + np.eye(setup.p)*__eps*cc) * np.exp(tau[i][t,it])
...
def calibPool(setup):
    """ Pooled Calibration Routine (Everyone gets the same theta!) """
    t0 = time.time()
    ...
    for m in range(1,setup.nmcmc):
        ...
        if m == 300:
            ...
            S   = (cov + np.eye(setup.p) * __eps) * cc * np.exp(tau)
        ...
        theta_cand  = (
            + theta[m-1]
            + np.einsum('ijk,ik->ij', np.linalg.cholesky(S), normal(size = (setup.ntemps, setup.p)))
            )
        ...
        if np.any(good_values):
            for i in range(setup.nexp):
                pred_cand[i][good_values] = setup.models[i].eval(tran2(theta_cand[good_values], setup.bounds_mat, setup.bounds.keys()))
                sse_cand[:, i] = np.sum((pred_cand[i] - setup.ys[i]) ** 2 / s2_vec_curr[i].T, 1)

        # for each temperature, accept or reject
        alpha = np.array([-9999.] * setup.ntemps)
        alpha[good_values] = (- 0.5 * setup.itl * (sse_cand[good_values] - sse_curr[good_values])).sum(axis = 1)
        ...
        if m % setup.decor == 0:
            for k in range(setup.p):
                ...
                good_values = setup.checkConstraints(
                    tran2(theta_cand, setup.bounds_mat,setup.bounds.keys()),
                    setup.bounds,
                    )
                #pred_cand = []
                #sse_cand = np.empty([setup.ntemps, setup.nexp])
            ...
            for _ in range(setup.ntemps): # do many swaps
                ...
                for i in range(setup.nexp):
                    alpha += (setup.itl[sw[1]] - setup.itl[sw[0]])*(
                            ...
                            - np.sum((setup.ig_a[i] + 1)*np.log(s2[i][m, sw[0], :]))
                            - np.sum(setup.ig_b[i]/np.log(s2[i][m, sw[0], :]))
                            + .5*np.sum(np.log(s2_vec_curr[i][:,sw[1]])) + 0.5*sse_curr[sw[1], i]
                            + np.sum((setup.ig_a[i] + 1)*np.log(s2[i][m, sw[1], :]))
                            + np.sum(setup.ig_b[i]/np.log(s2[i][m, sw[1], :]))
                            )
