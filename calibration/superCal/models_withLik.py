...
StatePoolMult = namedtuple(
    'StatePoolEmuMult', 'pred Sigma emu_vars cov',
    )
...
class AbstractModel:
    ...
    def llik(self, pred, cov): # assumes diagonal cov
        vec = self.yobs - pred 
...
class ModelBassPca(AbstractModel):
    ...
    def llik(self, pred, cov):
        vec = self.yobs - pred 
        out = -.5*(cov['ldet'] + vec.T @ cov['inv'] @ vec)
        return out

    def lik_cov_inv(self, s2vec):
        Sigma = cor2cov(self.meas_error_cor, np.sqrt(s2vec))
        chol = cholesky(Sigma + self.trunc_error_cov + self.discrep_cov + self.basis @ np.diag(self.emu_vars) @ self.basis.T)
        ldet = 2 * np.sum(np.log(np.diag(chol)))
        #la.dpotri(chol, overwrite_c=True) # overwrites chol with original matrix inverse
        inv=np.linalg.inv(Sigma + self.trunc_error_cov + self.discrep_cov + self.basis @ np.diag(self.emu_vars) @ self.basis.T)
        out = {'inv' : inv, 'ldet' : ldet}
        return out
