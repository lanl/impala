...
if __name__ == '__main__':
    ...
    def cf(x, bounds):
        good = (
            (x['sInf'] < x['s0']) * (x['yInf'] < x['y0']) * (x['y0'] < x['s0']) *
            (x['yInf'] < x['sInf']) * (x['s0'] < x['y1'])
            )
    ...
    # theta_ind = np.hstack([np.ones(v.T[0].shape, dtype = int) * i for i, v in enumerate(dat_all)])
    ...
    s2_ind = np.hstack([[v]*len(dat_all[v]) for v in range(nExp)])
    theta_ind = np.hstack([[v] * len(dat_all[v]) for v in range(nExp)])

    setup.addVecExperiments(yobs, model, sd_est, s2_df, s2_ind, theta_ind)
