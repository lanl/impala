...
edots = np.hstack((meta.edot[:nExp].values, meta.edot[:nExp].values))
temps = np.hstack((meta.temperature[:nExp].values, meta.temperature[:nExp].values))
## get SHPB experimental data
dat_all = []
for i in range(nExp):
    ## get first datset
    dat_all.append(pd.read_sql('select * from data_{};'.format(i + 1), con).values)
yobs = np.hstack([v.T[1] for v in dat_all] * 2)
sh = [v.T[0] for v in dat_all] * 2
