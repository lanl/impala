...
udf<-data.frame(u)
nn<-c('theta0', 'p', 's0','sinf',       'y0',      'yinf','y1', 'y2','kappa', 'gamma', 'vel')
...
png(paste('~/Desktop/cond',cond,'_parameters.png',sep=''),height=12*mult,width=14*mult,units='in',res=350/mult)
...
pred.vel<-do.call(rbind,mclapply(1:nrow(u),function(i) c(predict(mod.vel,t(unstandardize(u[i,])),n.cores = 1,mcmc.use = sample(1000,1))),mc.cores=nc))


pred.tc<-do.call(rbind,mclapply(1:nrow(u),function(i) c(predict(mod.tc,t(unstandardize(u[i,])),n.cores = 1,mcmc.use = sample(1000,1))),mc.cores=nc))
...
ss<-colMeans(sqrt(ps[[1]]$s2.hop[-burn,]))[1]

mult<-.65
png(paste('~/Desktop/cond',cond,'_predHB.png',sep=''),height=5*mult,width=8*mult,units='in',res=350/mult)
...
mult<-.7
png(paste('~/Desktop/cond',cond,'_predFP.png',sep=''),height=8*mult,width=8*mult,units = 'in',res=200/mult)
obs<-read.table('~/Desktop/immpala_data/Cu-Cu_Thomas_PlateImpact/ofhc-cu-symmetric-impact.txt')
...
png(paste('~/Desktop/cond',cond,'_predTC.png',sep=''),height=4*mult,width=8*mult,units = 'in',res=200/mult)
