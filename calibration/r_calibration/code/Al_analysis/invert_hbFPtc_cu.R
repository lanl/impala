...
load('~/Desktop/immpala_data/Copper_Taylor_Results_2019-05-23/features.rda')
...
get.sswe<-function(uu,tt,edot,w){
  mm<-mm.dat<-se<-list()
  ...
  return(sum(w*unlist(se)))
}


get.sswe.vel<-function(uu,w){
  #browser()
  uu<-unstandardize(uu)
  pred<-predict(mod.vel,uu[,c(1:5,6,8),drop=F],n.cores = 1,mcmc.use = sample(1000,1))
  sum(w*(pred-dat.vel.vec)^2)
}

get.sswe.tc<-function(uu,w){
  uu<-unstandardize(uu)
  pred<-predict(mod.tc,uu[,c(1:5,7,8),drop=F],n.cores = 1,mcmc.use = sample(1000,1))
  sum(w*(pred-dat.tc)^2)
}

get.alpha<-function(uu,s2.vel,s2.hop,s2.tc,w.vel,w.hop,w.tc,sswe.vel.curr,sswe.hop.curr,sswe.tc.curr){
  #browser()
  sswe.hop.cand<-get.sswe(uu,temps,edots,w.hop)
  sswe.vel.cand<-get.sswe.vel(uu,w.vel)
  sswe.tc.cand<-get.sswe.tc(uu,w.tc)
  alpha<- -.5*(sswe.hop.cand/s2.hop + sswe.vel.cand/s2.vel + sswe.tc.cand/s2.tc - sswe.hop.curr/s2.hop - sswe.vel.curr/s2.vel - sswe.tc.curr/s2.tc)
  return(list(alpha=alpha,sswe.hop=sswe.hop.cand,sswe.vel=sswe.vel.cand,sswe.tc=sswe.tc.cand))
}
...
mcmc<-function(nmcmc=30000,a.vel=0,b.vel=0,a.hop=0,b.hop=0,a.tc=0,b.tc=0){
  ...
  u[1,]<-start.u
  s2.vel<-s2.hop<-s2.tc<-sswe.vel<-sswe.hop<-sswe.tc<-NA
  ...
  w.vel<-rep(1/n.vel,n.vel)/3
  w.hop<-rep(1/n.hop,n.hop)/3
  w.tc<-rep(1/n.tc,n.tc)/3
    
  cand<-get.alpha(t(u[1,]),1,1,1,w.vel,w.hop,w.tc,1,1,1)
  sswe.vel[1]<-cand$sswe.vel
  sswe.hop[1]<-cand$sswe.hop
  sswe.tc[1]<-cand$sswe.tc
  ...
  for(i in 2:nmcmc){
    ...
    s2.vel[i]<-1/rgamma(1,n.vel/2+a.vel+sum(w.vel)/2,b.vel+.5*sswe.vel[i-1])
    s2.hop[i]<-1/rgamma(1,n.hop/2+a.hop+sum(w.hop)/2,b.hop+.5*sswe.hop[i-1])
    s2.tc[i]<-1/rgamma(1,n.tc/2+a.tc+sum(w.tc)/2,b.tc+.5*sswe.tc[i-1])
    
    u[i,]<-u[i-1,]
    sswe.hop[i]<-sswe.hop[i-1]
    sswe.vel[i]<-sswe.vel[i-1]
    sswe.tc[i]<-sswe.tc[i-1]
    ...
    if(constraints(model,u.cand)){ # constraint
      ...
      cand<-get.alpha(t(u.cand),s2.vel[i],s2.hop[i],s2.tc[i],w.vel,w.hop,w.tc,sswe.vel[i-1],sswe.hop[i-1],sswe.tc[i-1])
      alpha<-cand$alpha
    }
    if(log(runif(1)) < alpha){
      u[i,]<-u.cand
      sswe.hop[i]<-cand$sswe.hop
      sswe.vel[i]<-cand$sswe.vel
      sswe.tc[i]<-cand$sswe.tc
      count<-count+1
    }
    
    sswe.vel[i]<-get.sswe.vel(t(u[i,]),w.vel)
    sswe.tc[i]<-get.sswe.tc(t(u[i,]),w.tc)
  ...
  return(list(u=u,count=count,s2.vel=s2.vel,s2.hop=s2.hop,s2.tc=s2.tc,sswe.vel=sswe.vel,sswe.hop=sswe.hop,sswe.tc=sswe.tc))
}


model="JC"
ps<-mclapply(1:10,mcmc,nmcmc=30000,mc.preschedule = F,mc.cores = 10)
...
plot(dat.vel.vec,c(predict(mod.vel,t(unstandardize(a$u[30000,])),n.cores = 1,mcmc.use = 1))); abline(a=0,b=1,col=2)

dat.vel.pred<-matrix(c(predict(mod.vel,t(unstandardize(ps[[1]]$u[20000])[c(1:5,6,8)]),n.cores = 1,mcmc.use = 1)),nrow=1)
...
for(i in 29001:30000){
  k<-k+1
  dat.vel.arr[,,k]<-matrix(c(predict(mod.vel,t(unstandardize(ps[[1]]$u[i,])[c(1:5,6,8)]),n.cores = 1,mcmc.use = 1)),nrow=1)
}
...
for(i in 29001:30000){
  k<-k+1
  pred.tc.mat[k,]<-c(predict(mod.tc,t(unstandardize(ps[[1]]$u[i,])[c(1:5,7,8)]),mcmc.use=sample(1000,1)))
}
