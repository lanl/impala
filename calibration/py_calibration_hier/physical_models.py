...
#import ipdb
import copy
from math import pi, sqrt, log, erf, exp
...
class PTW_Yield_Stress(BaseModel):
    params = ['theta','p','s0','sInf','kappa','gamma','y0','yInf']
    consts = ['y1', 'y2', 'beta', 'matomic', 'chi']
    ...
    def value(self, edot):
        ...
        byield = mp.y1 * pow( mp.gamma * xiDot / edot , -mp.y2 )
        cyield = mp.s0 * pow( mp.gamma * xiDot / edot , -mp.beta)
...
class MaterialModel(object):
    ...
    def compute_state_history(self, strain_history):
        Nhist = len(strain_history)
        times = strain_history[:,0]
        strain_rate = np.diff(strain_history[:,1]) / np.diff(times)
        results = np.empty((Nhist, 6))

        state = self.state
        self.update_state(strain_rate[0], 0.)
        results[0] = [times[0], state.strain, state.stress, state.T, state.G, state.rho]
...
def generate_strain_history(emax, edot, Nhist):
    ...
    return np.vstack((times, strains)).T
