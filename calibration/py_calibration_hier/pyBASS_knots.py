...
class BassState2:
    ...
    def logProbChangeMod(self, n_int, vars_use, I_vec, z_vec, p, maxInt, knots_ind):
        if n_int == 1:
            out = (log(I_vec[n_int - 1]) - log(2 * p) + log(self.resid_dist[knots_ind])# proposal
                            + log(2 * p) + log(maxInt) - log(self.data.n))
        else:
            x = np.zeros(p)
            x[vars_use] = 1
            lprob_vars_noReplace = log(dmwnchBass(z_vec, vars_use))
    ...
    def genCandBasis(self, maxInt, I_vec, z_vec, p, xdata):
        n_int = int(np.random.choice(range(maxInt), p=I_vec) + 1)
        signs = np.random.choice([-1,1], size=n_int, replace=True)
        #knots = np.random.rand(n_int)
        knots = np.zeros(n_int)
        knots_ind = np.zeros(n_int)
        if n_int == 1:
            vs = np.random.choice(p)
            knots_ind = np.random.choice(self.data.n, size=1, p=self.resid_dist)
            knots = xdata[knots_ind,vs]
        else:
            vs = np.sort(np.random.choice(p, size=n_int, p=z_vec, replace=False))
            for i in range(n_int):
                knots_ind[i] = np.random.choice(self.data.n, size=1, p=self.resid_dist)
                knots[i] = xdata[knots_ind[i],vs[i]]
    
        basis = makeBasis(signs, vs, knots, xdata)
        lbmcmp = self.logProbChangeMod(n_int, vs, I_vec, z_vec, p, maxInt, knots_ind)
        return self.CandidateBasis(basis, n_int, signs, vs, knots, lbmcmp)

    def genBasisChange(self, knots, signs, vs, tochange_int, xdata):
        knots_cand = knots.copy()
        signs_cand = signs.copy()
        signs_cand[tochange_int] = np.random.choice([-1,1], size=1)
        knots_cand[tochange_int] = np.random.choice(xdata[:,vs[tochange_int]], size=1, p=self.resid_dist)#np.random.rand(1)
        basis = makeBasis(signs_cand, vs, knots_cand, xdata)
        return self.BasisChange(basis, signs_cand, vs, knots_cand)
    ...
    def update(self):
        ...
        if move_type == 1:
            ...
            cand = self.genCandBasis(self.prior.maxInt, self.I_vec, self.z_vec, self.data.p, self.data.xx)
            ...
            if log(np.random.rand()) < alpha:
                ...
                self.resid_dist = abs(self.data.y - np.dot(self.basis, self.bhat).reshape(self.data.n))
                self.resid_dist = self.resid_dist/sum(self.resid_dist)
            ...
            lbmcmp = self.logProbChangeMod(self.n_int[tokill_ind],self.vs[tokill_ind,0:self.n_int[tokill_ind]],I_vec,z_vec,self.data.p,self.prior.maxInt)
            ...
            cand = self.genBasisChange(self.knots[tochange_basis, 0:self.n_int[tochange_basis]], self.signs[tochange_basis, 0:self.n_int[tochange_basis]], self.vs[tochange_basis, 0:self.n_int[tochange_basis]], tochange_int, self.data.xx)
    ...
    def __init__(self, data, prior):
        ...
        self.cmod = False
        self.BasisChange = namedtuple('BasisChange', 'basis signs vs knots')
        self.CandidateBasis = namedtuple('CandidateBasis', 'basis n_int signs vs knots lbmcmp')
        self.resid_dist = abs(self.data.y - np.dot(self.basis, self.bhat).reshape(self.data.n))
        self.resid_dist = self.resid_dist/sum(self.resid_dist)
        return
...
class BassModel2:
    def __init__(self, data, prior, nstore):
        self.data = data
        self.prior = prior
        self.state = BassState2(self.data, self.prior)
        self.nstore = nstore
        s2 =  np.zeros(nstore)
        lam =  np.zeros(nstore)
        tau = np.zeros(nstore)
...
def bass2(xx, y, nmcmc = 10000, nburn = 9000, thin = 1, w1 = 5, w2 = 5, maxInt = 3, maxBasis = 1000, npart = None, g1 = 0, g2 = 0, s2_lower = 0, h1 = 10, h2 = 10, a_tau = 0.5, b_tau = None):
    if b_tau == None:
        b_tau = len(y)/2
    if npart == None:
        npart = min(20,.1*len(y))
    ...
    bp = BassPrior(maxInt, maxBasis, npart, g1, g2, s2_lower, h1, h2, a_tau, b_tau, w1, w2)
    nstore = int((nmcmc - nburn) / thin)
    bm = BassModel2(bd, bp, nstore) # if we add tempering, bm should have as many states as temperatures
    for i in range(nmcmc): # rjmcmc loop
        bm.state.update()
        if i > (nburn-1) and ((i - nburn + 1) % thin) == 0:
            bm.writeState()
...
if True:
    ...
    y = f(x) + np.random.normal(size=n, scale=.01)*0
    
    mod = bass(x,y,nmcmc=2000,nburn=500)
    mod2 = bass2(x,y,nmcmc=2000,nburn=500)
    
    plt.plot(np.sqrt(mod.samples.s2))
    plt.plot(np.sqrt(mod2.samples.s2))
