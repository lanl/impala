...
for i in range(n_shpb):
    ## get first datset
    dat_all.append(
        pd.read_sql("select * from data_{};".format(i + 1), con).values
    )
...
if emu:
    ...
    for i in np.where(meta["type"] != "shpb")[0][range(n_tc)]:
        X = pd.read_sql(
            "select * from sims_input_{};".format(i + 1), con
        ).values
        X[:, 9] = np.log(X[:, 9])  # we use log gamma below
        y = pd.read_sql(
            "select * from sims_output_{};".format(i + 1), con
        ).values
...
sd_est_shpb = np.array([0.00025] * n_shpb)
...
model_ptw_hier = sc.ModelMaterialStrength(
...
if emu:
    ...
    for i in range(n_tc):
        models_emu.append(
            sc.ModelBassPca_func(
                emu_list[i],
                input_names_emu,
                exp_ind=np.array([0] * len(y_emu_list[i])),
                s2="fix",
            )
        )
    for i in range(n_tc):
...
setup_hier_ptw.setMCMC(nmcmc=30000, thin=1, decor=100, start_tau_theta=-4.0)
setup_hier_ptw.setHierPriors(
    ...
    Sigma0_prior_scale=np.eye(setup_hier_ptw.p)
    * 0.1
    ** 2,  # used .1**2 before, .5 is what we used elsewhere, indicating low shrinkage (may want to use .5**2 or .25**2 instead), but that was probit space
)
...
pp.save_parent_strength(
    setup_hier_ptw,
    setup_hier_ptw.models[0],
    out_hier,
    mcmc_use,
    path + "parent_" + name + ".csv",
)  # saves parent distribution
...
pp.parameter_trace_plot(
    out_hier.theta0[:, 0], ylim=[0, 1]
)  # we want these to look like they converge, choose burn-in accordingly
plt.savefig(path + "traceTheta0_" + name + ".pdf")

pp.pairs(
    setup_hier_ptw, out_hier.theta0[mcmc_use, 0]
)  # pairs plot of posterior samples of theta0 (parent mean)
plt.savefig(path + "samplePairsTheta0_" + name + ".pdf")

pp.parameter_trace_plot(
    out_hier.theta[0][:, 0, 4], ylim=[0, 1]
)  # trace plot of the 5th experiment parameters
plt.savefig(path + "traceTheta5_" + name + ".pdf")

pp.pairs(
    setup_hier_ptw, out_hier.theta[0][mcmc_use, 0, 4]
)  # pairs plot of the 5th experiment parameters
plt.savefig(path + "samplePairsTheta5_" + name + ".pdf")

pp.pairwise_theta_plot_hier(
    setup_hier_ptw, out_hier, path + "pairs_" + name + ".pdf", mcmc_use
)  # saves the combined pairs plot
...
pp.get_best_sse(
    path + "parent_" + name + ".csv", path + "bestSSE_" + name + ".csv"
)  # function to get best parameters (uses sum of squared error, sse)
pp.get_bounds(
    edot=1.1e6,
    strain=1.0,
    temp=0.2 * 2400,
    results_csv=path + "parent_" + name + ".csv",
    write_path=path + "bounds_" + name + ".csv",
