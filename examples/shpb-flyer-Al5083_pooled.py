...
out.multiple_align_functions(x1, parallel=True, lam=0.01)
...
for j in range(nexp):
    ...
    out.multiple_align_functions(sims_all_list[j][0], parallel=True, lam=0.01)
    ...
    out2 = fs.pairwise_align_functions(
        sims_all_list[j][0], obs_all_list[j], xx_all_list[j], lam=0.01
    )
...
for j in range(nexp):
    ...
    axis[j, 2].plot(
        xx,
        gam_sim_list[j] * (np.max(xx_all_list[j]) - np.min(xx_all_list[j]))
        + np.min(xx_all_list[j]),
        color="lightgrey",
    )
    axis[j, 2].plot(
        xx,
        gam_obs_list[j] * (np.max(xx_all_list[j]) - np.min(xx_all_list[j]))
        + np.min(xx_all_list[j]),
    )
...
ftilde_train_list = [
    np.delete(ftilde_sim_list[j], ho, axis=1) for j in range(nexp)
]
...
bounds_mat = np.array(
    [[sim_inputs[:, i].min(), sim_inputs[:, i].max()] for i in range(p)]
)
...
for j in range(nexp):
    model_ftilde = sc.ModelBassPca_func(
        emu_ftilde_list[j], input_names
    )  # for aligned data
    model_vv = sc.ModelBassPca_func(
        emu_vv_list[j], input_names
    ...
    setup.addVecExperiments(
        ...
        D=np.hstack(
            (
                np.vstack(
                    (np.ones([starts[j], 1]), np.zeros([M - starts[j], 1]))
                ),
                np.vstack(
                    (
                        np.zeros([starts[j], 1]),
                        np.ones([M - starts[j] - ends, 1]),
                        np.zeros([ends, 1]),
                    )
                ),
                np.vstack(
                    (
                        np.zeros([starts[j], 1]),
                        np.linspace(-1, 1, M - starts[j] - ends)[:, None],
                        np.zeros([ends, 1]),
                    )
                ),
...
theta_dict = sc.tran_unif(
    out.theta[uu, 0, :], setup.bounds_mat, setup.bounds.keys()
)
ftilde_pred_obs = [
    setup.models[j].eval(theta_dict) for j in range(0, nexp * 2, 2)
]
vv_pred_obs = [
    setup.models[j].eval(theta_dict) for j in range(1, nexp * 2, 2)
]  # note, it seems like adding discrepancy here doesn't really change anything
gam_pred_obs = [fs.geometry.v_to_gam(x.T) for x in vv_pred_obs]
gamd_pred_obs = [
    fs.geometry.v_to_gam(
        vv_pred_obs[j].T
        + setup.models[j * 2 + 1].D @ out.discrep_vars[j * 2 + 1][uu, 0].T
    )
    for j in range(nexp)
]
...
for j in range(nexp):
    ...
    plt.legend(
        ["model runs", "calibrated predictions", "experiment"],
        loc="upper left",
        prop={"size": 6},
    )
ax = plt.gca()
...
for j in range(nexp):
    axis[j].plot(
        xx,
        gam_train_list[j] * (np.max(xx_all_list[j]) - np.min(xx_all_list[j]))
        + np.min(xx_all_list[j]),
        color="lightgrey",
    )
    ...
    axis[j].plot(
        xx,
        gamd_pred_obs[j] * (np.max(xx_all_list[j]) - np.min(xx_all_list[j]))
        + np.min(xx_all_list[j]),
        color="lightblue",
    )
    # plt.plot(ftilde_train[:,ind_true],color='black')
...
for j in range(nexp):
    for i in range(len(uu)):
        obspred_all_list[j][i] = fs.warp_f_gamma(
            xx_all_list[j],
            ftilde_pred_obs[j][i, :],
            fs.invertGamma(gamd_pred_obs[j][:, i]),
        )
