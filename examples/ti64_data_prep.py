...
def load_Ti64_shpb(curs):
    ...
    for paper in papers:
        ...
        for file in files:
            ...
            data = pd.read_csv(file, skiprows=range(17)).values
    ...
    for xp in xps:
        # Set the ID
        ti64_curr_id += 1
        table_name = f"data_{ti64_curr_id}"
...
for xp in xps:
    ti64_curr_id += 1
    table_name = f"data_{ti64_curr_id}"
    emu_iname = f"sims_input_{ti64_curr_id}"
    emu_oname = f"sims_output_{ti64_curr_id}"
