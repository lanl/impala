#########################################
#########################################
### Beryllium Hierarchical Calibration ###
### PTW - BP
#########################################
#########################################
...
#from sklearn.metrics import r2_score #Can't install the package, Sep. 10, 2025
 
path_to_dir = '/Users/jyplohr/impala_runs/'
path_to_data = '/Users/jyplohr/impala_runs/Be/data/QS_SHPB_Z_2017/'
path_to_results = '/Users/jyplohr/impala_runs/Be/PTW-BP/hierarchical_zmachine/results/'
path_to_plots = '/Users/jyplohr/impala_runs/Be/PTW-BP/hierarchical_zmachine/plots/'
...
s200F_1 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_293K_1850s_clean_Mbar.txt")
s200F_2 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_295K_0.001s_clean_Mbar.txt")
s200F_3 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_295K_1s_clean_Mbar.txt")
s200F_4 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_473K_0.001s_clean_Mbar.txt")
s200F_5 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_473K_1s_clean_Mbar.txt")
s200F_6 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_473K_2000s_clean_Mbar.txt")
s200F_7 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_673K_0.001s_clean_Mbar.txt")
s200F_8 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_673K_1s_clean_Mbar.txt")
s200F_9 = np.loadtxt(path_to_data + "../S200F/Be_S200F_new_673K_3000s_clean_Mbar.txt")
s200F_10 = np.loadtxt(path_to_data + "../S200F/Be_S200F_old_293K_1900s_clean_Mbar.txt")
s200F_11 = np.loadtxt(path_to_data + "../S200F/Be_S200F_old_296K_0.001s_clean_Mbar.txt")
s200F_12 = np.loadtxt(path_to_data + "../S200F/Be_S200F_old_473K_0.001s_clean_Mbar.txt")
s200F_13 = np.loadtxt(path_to_data + "../S200F/Be_S200F_old_473K_2200s_clean_Mbar.txt")
s200F_14 = np.loadtxt(path_to_data + "../S200F/Be_S200F_old_673K_0.001s_clean_Mbar.txt")
s200F_15 = np.loadtxt(path_to_data + "../S200F/Be_S200F_old_673K_2700s_clean_Mbar.txt")
...
#### New BGP Parameterization from JeeYeon 01/29/25
...
consts_ptw = {
   ...
   #Piecewise quadratic specific heat
    'T_t'   : 525, #K, transition temperature##
...
## PW shear modulus model with lienar cold shear from sesame 32023, unit:Mbar
    #'G0'     : 1.527,
#    #'g0'     : -1.12157,
#    #'g1'     : 1.43,   
#    #'alpha'  : 0.199
...
class PTWbp_Yield_Stress(impala.physics.BaseModel):
    ...
    def value(self, edot):
        ...
        ### Lauren added this!!!!!
        tau_y[tau_y > tau_s] = tau_s[tau_y > tau_s]
        ...
        if (np.any((1.0 - (1.0 - np.exp(- eArg1)) * np.exp(-eArg2)) <= 0) or \
            ...
            print('bad')
            print((tau_s-tau_y)[ind].min())
            print((tau_s-tau_y)[ind].max())
...
model_hier_ptw = sc.ModelMaterialStrength(temps=np.array(temps), 
        ...
        strain_histories=[strain_hist_list[j][inds_subsampled[j]] for j in range(len(dat_all))],  #Note: implementing subsampling!!!!
        flow_stress_model='PTWbp_Yield_Stress',
        melt_model= 'BGP_Melt_Temperature_Fixed',
        shear_model= 'BGP_PW_Shear_Modulus_Fixed',
        specific_heat_model='Piecewise_Quadratic_Specific_Heat', 
...
model_z = [sc.ModelMaterialStrength(temps=np.array(temps_z)[j], 
        ...
        flow_stress_model='PTWbp_Yield_Stress',
        melt_model= 'BGP_Melt_Temperature_Fixed', #same as main
        shear_model= 'BGP_PW_Shear_Modulus_Fixed',
...
THETA0_Y = get_outcome_predictions_impala(setup_hier_ptw, theta_input = np.array(pd.DataFrame(sc.tran_unif(np.asarray(df0)[uu,:],setup_hier_ptw.bounds_mat, setup_hier_ptw.bounds.keys()).values())).T)['outcome_draws']
...
j=1
fig,ax=plt.subplots(1,1,figsize=(6,4), sharey = False)    
ax.fill_between(np.append(np.append(setup_hier_ptw.models[j].meas_strain_histories[0]-NUDGE, setup_hier_ptw.models[j].meas_strain_histories[0]), setup_hier_ptw.models[j].meas_strain_histories[0]+NUDGE),
                np.repeat(QUANTS_PARENT_Y[j][0,:].flatten(),3), np.repeat(QUANTS_PARENT_Y[j][2,:].flatten(),3), color = 'darkgray', zorder = 1)            
ax.plot(setup_hier_ptw.models[j].meas_strain_histories[0],QUANTS_PARENT_Y[j][1,:].flatten(), color = 'red', zorder = 2, linewidth = 2, label = 'Parent')
ax.fill_between(np.append(np.append(setup_hier_ptw.models[j].meas_strain_histories[0]-NUDGE, setup_hier_ptw.models[j].meas_strain_histories[0]), setup_hier_ptw.models[j].meas_strain_histories[0]+NUDGE),
                np.repeat(QUANTS_THETAi_Y_z1[j][0].flatten(),3), np.repeat(QUANTS_THETAi_Y_z1[j][2].flatten(),3), color = 'red', zorder = 1)            
ax.plot(setup_hier_ptw.models[j].meas_strain_histories[0],QUANTS_THETAi_Y_z1[j][1].flatten(), color = 'red', zorder = 4, linewidth = 2, label = 'Experiment-Specific')
ax.scatter(setup_hier_ptw.models[j].meas_strain_histories[0],setup_hier_ptw.ys[j], color = 'black', zorder = 3, label = 'Data')
ax.set_ylabel('Stress')
ax.set_xlabel('Strain')
ax.set_xlim(setup_hier_ptw.models[j].meas_strain_histories[0]-20*NUDGE, setup_hier_ptw.models[j].meas_strain_histories[0]+20*NUDGE)
...
for i in range(2):
    for j in range(6):
        ...
        ax[i,j].set_ylabel('')
        ax[i,j].set_title(KEYS[6*i+j]) 
        A = sns.boxplot(data = theta_i_long, x = theta_i_long['exp'], y = theta_i_long[KEYS[6*i+j]], hue = theta_i_long['Category'],ax=ax[i,j],showfliers=False, palette = palette, width = 1, dodge = False)
...
for i in range(2):
    for j in range(6):
        ...
        ax[i,j].set_ylabel('')
        ax[i,j].set_title(KEYS[6*i+j]) 
        A = sns.boxplot(data = theta_i_long, x = theta_i_long['strain_ind'], y = theta_i_long[KEYS[6*i+j]], hue = theta_i_long['Category'], ax=ax[i,j],showfliers=False, palette = palette, width = 1, dodge = False)
        A.set(xlabel='Experiment',ylabel='')
        A.set(xticklabels=[]) 
        if i > 0 or j > 0:
            ax[i,j].get_legend().set_visible(False)
...
MEDS = theta_i_long[['theta', 'p', 's0', 'sInf', 'kappa', 'lgamma', 'y0', 'yInf', 'y1', 'y2', 'lgammas', 'm']+['exp']].groupby('exp').mean()
MEDS = np.array(MEDS)
KEYS = np.array(pd.DataFrame(setup_hier_ptw.bounds.keys())).flatten()

purity_long = np.append(np.asarray(purity), np.repeat('Lower',5))
...
for i in range(2):
    for j in range(6):
        ...
        ax[i,j].set_title(KEYS[6*i+j]) 
        sns.scatterplot(x=np.append(np.log10(edots), np.log10(edots_z)), y=MEDS[:,6*i+j], ax = ax[i,j], color = 'orange')
        # if i > 0 or j > 0:
        #     ax[i,j].get_legend().set_visible(False)
...
for i in range(2):
    for j in range(6):
        ...
        ax[i,j].set_ylabel('Post. Mean')
        ax[i,j].set_title(KEYS[6*i+j]) 
        sns.scatterplot(x=np.append(np.asarray(temps), np.asarray(temps_z)), y=MEDS[:,6*i+j], ax = ax[i,j],  color = 'orange')
        # if i > 0 or j > 0:
        #     ax[i,j].get_legend().set_visible(False)
...
ax.plot(np.arange(0,len(OBS)),np.hstack(pred_median[0]).flatten(), color = 'purple', zorder = 2, linewidth = 2, label = 'Posterior Median')
ax.plot(np.arange(0,len(OBS)),np.hstack(pred_minsse[0]).flatten(), color = 'red', zorder = 2, linewidth = 2, label = 'Best SSE')
