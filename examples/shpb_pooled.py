...
model_jc = sc.ModelMaterialStrength(
...
setup_pool_ptw.setMCMC(
    nmcmc=10000, nburn=5000, thin=1, decor=100, start_tau_theta=-4.0
)

setup_pool_jc = sc.CalibSetup(bounds_jc, constraints_jc)
setup_pool_jc.addVecExperiments(
    yobs=stress_stacked,
    model=model_jc,
    sd_est=sd_est,
    s2_df=s2_df,
...
mat = np.vstack((
    out_pool.theta[uu, 0, :],
    out_pool.theta[uu[mle_idx], 0, :],
    out_pool.theta[uu, 0, :].mean(0),
))

##########################################################################################
# posterior predictions (without measurement error, which has standard
# deviation np.sqrt(out.s2)), disregarding the first 25000 MCMC samples
pred = setup_pool.models[
    0
].eval(  # evaluates the model passed to setup.  If more than one call to "addExperiment..." use the index
    # transforms thetas (which are scaled to 0-1) to native scale, results in a
    # dict
    sc.tran_unif(mat, setup_pool.bounds_mat, setup_pool.bounds.keys())
)  # note, our PTW eval doesn't care about parameter ordering, just the names in the dict

plt.plot(
    pred.T, color="grey"
)  # note, these don't include the additional error we are estimating (measurement error, plus everything else)
